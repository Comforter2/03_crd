...
ggplot(crd_asw, aes(x= nrate_kgha,
  ...
  geom_jitter() +
  theme_bw() +
  theme(legend.position = "none") # creating box plot to first understand nitrogen effects on yield
...
```
From the ANOVA results, only the main effects of nirogen and potassium were significant. No significant interactive effect between treatment factors was observed.
...
```
All p values were above 0.05, hence the assumption is met for normal distribution and double checked for homoscedasticity
